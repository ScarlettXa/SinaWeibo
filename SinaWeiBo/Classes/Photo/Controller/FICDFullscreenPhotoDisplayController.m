...
@interface FICDFullscreenPhotoDisplayController () <UIGestureRecognizerDelegate,UIScrollViewDelegate> {
...
- (id)init {
    ...
    if (self != nil) {
        ...
        _fullscreenView = [[UIScrollView alloc] init];
...
- (void)showFullscreenPhoto:(FICDPhoto *)photo forImageFormatName:(NSString *)imageFormatName withThumbnailImageView:(UIImageView *)thumbnailImageView {
    ...
    MyLog(@"The display pic's frame is:%f",convertedThumbnailImageViewFrame.size.width);
    ...
    _fullscreenView.contentSize=sourceImage.size;
    //设置最大伸缩比例
    _fullscreenView.maximumZoomScale=15.0;
    //设置最小伸缩比例
    _fullscreenView.minimumZoomScale=0.5;
